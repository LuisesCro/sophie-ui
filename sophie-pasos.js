...
(function (global) {
  ...
  var PASOS = {
    ...
    // LAS PANTALLAS MANUALES VUELVEN, y conviene dejar escrito por que se
    // borraron y por que estan de nuevo aqui.
    //
    // Se borraron porque tener DOS versiones de cada pantalla y una condicion
    // para elegir significaba que cualquier cosa que dejara esa condicion en
    // false —una variable sin poner, una sesion sin correo, un campo perdido en
    // un puente— devolvia al estudiante al guion de Helium 10 SIN QUERER. Y
    // volvio, una y otra vez, durante semanas.
    //
    // Pero el razonamiento tenia un agujero: daba por hecho que nadie necesita
    // ya ese flujo. Y no es cierto. Los estudiantes que hoy estan en el curso NO
    // tienen datos de Jungle Scout —esa parte esta en pruebas— y borrarles las
    // pantallas les dejo un paso 3 que no lleva a ningun sitio. Una estudiante
    // lo reporto el mismo dia.
    //
    // Lo que estaba mal no era tener dos caminos: era que se cayera al segundo
    // por accidente. Eso ya no pasa, y por dos motivos: la puerta DATOS_AMAZON
    // se configura a proposito (con una lista de correos mientras esto se
    // prueba), y el prompt ya no nombra ninguna herramienta, asi que un fallo de
    // la señal no puede hacer que Sophie improvise un recorrido de Helium 10.
    //
    // Este camino es AHORA una eleccion, no un despeñadero.
